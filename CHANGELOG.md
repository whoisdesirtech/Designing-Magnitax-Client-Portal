...
- **Real TOTP 2FA (Identity Platform, Option A)** — the fake 6-digit 2FA gate is replaced with real TOTP multi-factor authentication using the modular `firebase/auth` SDK (loaded alongside compat via `window.__mfa`). Login with an enrolled account returns `auth/multi-factor-auth-required`, which the 2FA view resolves with `TotpMultiFactorGenerator.assertionForSignIn`. Settings gains a live enrollment panel: QR code + manual secret (TOTP secret from `generateQrCodeUrl`, rendered via the free qrserver API), verify-and-enable, and disable (unenroll). Dashboard 2FA stat and Settings card reflect real enrollment state.

### Changed

- **2FA gate is now real** — the 2FA screen only appears when a pending MFA challenge exists; accounts without enrolled factors (and returning persisted sessions) go straight into the app. Requires the Firebase Authentication **Identity Platform** upgrade in the console (enabled 2026-08-15; free tier 50K MAU/month on Blaze).
